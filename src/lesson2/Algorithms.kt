...
// В первом 7к символов, во втором 6к (пусть тогда N = 6к). Максимальное значение, которое принимает K (высчитал с отладчиком), равно 928 ~ 1к.
// Тогда временную сложность в данном случае можно оценить как: O(N * (N + K) = O(N * (N + N/6) = O(N^2)
// Наконец, на решение этого теста с текстами из поэмы "Руслан и Людмила" на моём ноутбуке тратится меньше секунды,
// в то время как при лобовом решении я не могу даже дождаться окончания исполнения программы.
//
// В худшем варианте (когда first полностью равен second), K = (1 + N)/2 * N, и тогда сложность можно оценить как O(N^3)
// Однако даже в таком случае K достигает (1 + N)/2 * N лишь на первой итерации основного цикла. На каждой последующей итерации
// K будет иметь меньшее значение, чем на предыдущей итерации.
